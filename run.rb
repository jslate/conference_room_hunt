...
#  9) add loading of next level when all appointments are collected
...
class Player
  ...
  def initialize(map, x, y)
    ...
    @standing, @walk1, @walk2, @jump = *Gosu::Image.load_tiles("media/guy.png", 50, 50)
  ...
  def collect_appointments(appointments)
    # Same as in the tutorial game.
    appointments.reject! do |c|
      (c.x - @x).abs < 50 and (c.y - @y).abs < 50
    end
  end
end

# Map class holds and draws tiles and appointments.
class Map
  attr_reader :width, :height, :appointments
    ...
    @tiles = Array.new(@width) do |x|
      Array.new(@height) do |y|
        ...
        when '#'
          Tiles::Plant
        when 'x'
          @appointments.push(Appointment.new(appointment_img, x * 50 + 25, y * 50 + 25))
          nil
        else
          nil
        end
...
class ConferenceRoomHunt < (Example rescue Gosu::Window)
  def initialize
    super WIDTH, HEIGHT

    self.caption = "Conference Room Hunt"

    @sky = Gosu::Image.new("media/background.png", :tileable => true)
  ...
  def update
    ...
    @cptn.update(move_x)
    @cptn.collect_appointments(@map.appointments)
...
ConferenceRoomHunt.new.show if __FILE__ == $0
